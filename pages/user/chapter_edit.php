<?PHP
	...
	//SQL Scripts
	$query = mysqli_query($connect,"SELECT * FROM info_chapter WHERE id = $idnum");

	//Fetch Equipment Information
	while($row = mysqli_fetch_assoc($query)){
		$id = $row['id'];
		$Sector_Name = $row['chapterName'];
    $areaID = $row['sectorId'];
	}
?>
<!DOCTYPE html>
      ...
      <div class="col-lg-12" >
        <div class="panel panel-success">
          ...
          <div class="panel-body">
            <form action="database/edit_chapter.php" enctype="multipart/form-data" method="post">
              <div class="form-group">
                <div class="row">
                  <div class="col-lg-12">
                    <label>Chapter ID</label>
                    <input class="form-control" name="ChapterID" value="<?PHP echo $id;?>" readonly>
                    <label>Sector Name</label>
    								<select class="form-control" name="Sector">
    									<option value="">Please Select Sector</option>
    								<?PHP
    									$connect = mysqli_connect($host, $user, $pass,$databasename) or die("Couldn't connect to database!");
    									$area = mysqli_query($connect, "SELECT * FROM info_sector");
    									while($row = mysqli_fetch_assoc($area)){
    										if($row['id'] == $areaID) {
    											echo "<option value = ". $row['id']." selected>".$row['sectorName']."</option>";
    										}
    										else {
                    ...
                    <input class="form-control" name="ChapterName" value="<?PHP echo $Sector_Name;?>" required>
                    <br>
                    <center>
